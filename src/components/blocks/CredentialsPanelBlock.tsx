      ...
      <div className="container-main px-6 lg:px-20">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {content.panels.map((panel, i) => (
            <div
              key={i}
              className="rounded-xl border border-gray-200 bg-gray-50 p-5 transition-all duration-300 hover:border-transparent hover:shadow-[0_16px_32px_-16px_rgba(31,56,100,0.2)]"
            >
              ...
              {panel.note && (
                <p className="font-arial text-gold-500 text-xs mt-1">{panel.note}</p>
              )}
              {panel.note2 && (
