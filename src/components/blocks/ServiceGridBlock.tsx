  ...
  return (
    ...
    >
      {/* Custom-quote services get a small marker distinguishing them as
          ...
          presentation. */}
      {isCustom && (
        <div className="absolute -top-2.5 right-5 rounded-full bg-gradient-to-br from-gold-200 to-gold-300 px-3 py-1 text-[10px] font-arial font-bold uppercase tracking-wide text-navy-600 shadow-sm">
          Bespoke
        </div>
      )}
      ...
      <div
        className={`w-12 h-12 rounded-xl flex items-center justify-center mb-4 transition-colors duration-300 ${
          isCustom ? 'bg-gold-200/15 group-hover:bg-gold-200/25' : 'bg-crimson-50 group-hover:bg-crimson-100'
        }`}
      >
        <IconComponent
          size={24}
          className={isCustom ? 'text-gold-300' : 'text-crimson-400'}
          strokeWidth={1.75}
        />
      </div>
      ...
      <Link
        ...
        className={`inline-flex items-center justify-center w-full text-center mt-4 px-4 py-2.5 rounded-lg font-arial text-sm font-semibold transition-all duration-300 ${
          isCustom
            ? 'bg-gradient-to-br from-gold-200 to-gold-300 text-navy-600 hover:shadow-[0_6px_16px_rgba(212,169,79,0.35)]'
